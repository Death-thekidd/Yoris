...
import {View, SafeAreaView, Text, StyleSheet, TextInput, FlatList, Image, ScrollView, TouchableOpacity, Alert} from 'react-native';
...
export default function Ecommerce5({navigation, route}){
	...
	<View style={{right:0, marginRight:20, marginBottom:30, bottom:0, position:'absolute', zIndex:1}}><View style={styles.message}><Icon4 name="message1" size={25} color="#000" /></View></View>
	<ScrollView
	...
	<View style={styles.buyNow}><StyledButton onPress={()=>navigation.navigate('Ecommerce8')}><BuyNow>Buy Now</BuyNow></StyledButton></View>
	<View style={styles.buyNow}><StyledButton2  onPress={()=>navigation.navigate('Ecommerce6')}><Add>Add to Cart</Add></StyledButton2></View>
		...
		<FlatList
						...
						renderItem={({item, index})=>
							<TouchableOpacity style={styles.imageView} onPress={()=>navigation.navigate('Ecommerce3', {image:item.image, store:item.store})}>
								...
								<Image source={item.image} style={styles.image2} key={index}/>
							</TouchableOpacity>
						}
						/>
	</View>
